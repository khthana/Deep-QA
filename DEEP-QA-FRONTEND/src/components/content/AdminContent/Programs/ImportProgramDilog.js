  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/api/programs/import-programs`,
        {
          method: 'POST',
          body: formData,
          credentials: 'include',
      ...
      if (res.ok) {
        setAlert({
          open: true,
          message: `นำเข้าไฟล์หลักสูตร สำเร็จ`,
          severity: 'success',
        })
      } else {
        setAlert({
          open: true,
          message: `นำเข้าไฟล์หลักสูตร ไม่สำเร็จ`,
          severity: 'error',
        })
      }
        ...
        <ContentMotionDIV className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="relative mb-48 w-[90%] max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="my-4 flex flex-col items-center">
              <button
                onClick={handleDownloadProgramTemplate}
...
export default ImportProgramDialog

const handleDownloadProgramTemplate = () => {
  const headers = ['program_id', 'program_name_en', 'program_name_th', 'year']
  ...
  worksheet['!cols'] = [
    { wch: 10 }, // ID
    { wch: 30 }, // program_name_en
    { wch: 30 }, // program_name_th
    { wch: 10 }, // year
  ]
  ...
  saveAs(blob, 'program-template.xlsx')
}
