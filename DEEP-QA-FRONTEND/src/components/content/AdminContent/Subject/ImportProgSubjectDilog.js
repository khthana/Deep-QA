...
function ImportProgSubjectDialog({
  isOpen,
  onClose,
  setAlert,
  fetchSubjects,
  ...
  const handleUpload = async () => {
    ...
    formData.append('program_id', SelectedProg.program_id)
    formData.append('email', profile.email)
    // console.log(SelectedProg.program_id, profile.email)
    onClose()
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/api/program_subjects/import-program-subject`,
        {
          method: 'POST',
          body: formData,
          credentials: 'include',
      ...
      if (res.ok) {
        setAlert({
          open: true,
          message: `นำเข้าไฟล์รายวิชาในหลักสูตร สำเร็จ`,
          severity: 'success',
        })
      } else {
        setAlert({
          open: true,
          message: `นำเข้าไฟล์รายวิชาในหลักสูตร ไม่สำเร็จ`,
          severity: 'error',
        })
      }
          ...
          <ContentMotionDIV className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div className="relative mb-48 w-[90%] max-w-md rounded-2xl bg-white p-6 shadow-2xl">
              <div className="mb-2 text-center text-2xl text-secondary">
                อัปโหลดไฟล์รายวิชาในหลักสูตร
              </div>
              <p className="text-center text-sm text-gray-600">
                กรุณาอัปโหลดไฟล์{' '}
                <span className="font-medium">CSV, Excel หรือ JSON</span>
                <br />
                สำหรับเพิ่มรายวิชาในหลักสูตร
              </p>

              <div className="my-4 flex flex-col items-center">
                <button
                  onClick={handleDownloadSubjectTypeTemplate}
...
export default ImportProgSubjectDialog

const handleDownloadSubjectTypeTemplate = () => {
  const headers = ['subject_id', 'subject_type']
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'SubjectTypes')
  ...
  saveAs(blob, 'subject-type-template.xlsx')
}
