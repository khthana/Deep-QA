...
function ImportStudentGroupsDialog({
  isOpen,
  onClose,
  setAlert,
  fetchStudentGroup,
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/api/studentGroup/import-student-groups`,
        {
          method: 'POST',
          body: fd,
          credentials: 'include',
      ...
      if (res.ok) {
        setAlert?.({
          open: true,
          message: 'นำเข้ากลุ่มนักศึกษา สำเร็จ',
          severity: 'success',
        })
      } else {
        setAlert?.({
        ...
        >
          <div className="relative mb-48 w-[90%] max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <p className="text-center text-sm text-gray-600">
              ...
              <br />
              สำหรับเพิ่มกลุ่มงานนักศึกษาในรายวิชา
            </p>
...
export default ImportStudentGroupsDialog
...
const handleDownloadGroupTemplate = () => {
  ...
  saveAs(blob, 'group-template.xlsx')
}
