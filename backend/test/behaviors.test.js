...
test('an id too large for the column is ไม่พบ, not a 500', async () => {
  ...
  for (const id of OVERWIDE_IDS) {
    ...
    assert.equal(answered.body.message, REFUSALS.behaviorNotFound);

    // The Section and the CLO are `clos.js`'s guards, reached from here.
    const bySection = await list(cookie, id, clo.clo_id);
    assert.equal(bySection.status, 404, id + ' section answered ' + bySection.status);
    const byClo = await list(cookie, sectionId, id);
