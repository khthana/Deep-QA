...
- **Tier 2 — junctions and assignments** (`program_subjects`, `subject_plo_mapping`, `user_roles`, `student_course`,
  `student_group_member`, `rubrics`): the natural key is 2–3 short columns, so the surrogate `id` is dropped and the
...
- **Tier 3 — deeply nested records** (`semester_courses`, `course_sections`, `subject_clo`, `subject_score_ratio`,
  `activities`, `course_syllabus`, and the log/evidence tables): a surrogate primary key is kept for FK and URL use,
  but the full natural key is enforced with a `UNIQUE` constraint.
