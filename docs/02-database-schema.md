...
| การลบ | — | `CASCADE` ห้าคอลัมน์ — `student_group_member.group_id` และ `activity_clo_mapping`, `activity_scores`, `activity_evidence`, `rubric_details` ที่ชี้ไปพ่อแม่ของตัวเอง · ที่เหลือ RESTRICT · authorship / uploader `SET NULL` · `activities.course_syllabus_id` `SET NULL` | กฎเดียวกับ 0001 · `student_group` เป็น RESTRICT แม้กลุ่มนอกกลุ่มเรียนจะไร้ความหมาย เพราะการลบกลุ่มเป็นการกระทำที่ต้องถูกบันทึก cascade จะลบกลุ่มโดยไม่ผ่านโค้ดที่เขียน log · **สามคอลัมน์ที่ห้อยจากกิจกรรมต้อง CASCADE พร้อมกัน** เพราะ `deleteActivity` ยิง `DELETE FROM activities WHERE id = $1` เปล่า ๆ ไม่เก็บกวาดลูกก่อน RESTRICT ที่คอลัมน์ใดคอลัมน์หนึ่งจึงทำให้ endpoint นั้นพังทันทีที่กิจกรรมมีคะแนนหรือมีหลักฐาน · แถวหลักฐานที่ถูก cascade ทิ้งไฟล์ไว้บนดิสก์ ซึ่งไม่มีโค้ดเดิมส่วนไหนลบไฟล์อยู่แล้ว และไฟล์ที่ไม่มีแถวคือครึ่งที่กู้คืนได้ · `activity_evidence` ยังคง soft-delete (`is_deleted`) ในเส้นทางปกติ เพราะเป็นสิ่งที่การประเมิน TABEE ถูกนำมาแสดง |
