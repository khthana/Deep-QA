...
* `finally` ของการอ่านที่ถูกแทนยังตั้ง `busy` เป็น false ระหว่างที่งานอื่นยังไม่กลับ - มีมาก่อน
  #139 และ guard แบบเดียวกันใช้ไม่ได้ (เพิ่มกับลบไม่ได้ตั้ง `busy` เอง) จึงแยกเป็น #142
  ซึ่งแก้แล้ว - ดู `mutation/142-busy-outlives-a-superseded-read.py`
* ประโยคของการเขียนตอนเรียนบนแผงหนึ่ง ขึ้นเหนือแผงของ Offering อื่นได้ - เป็นคำถามเรื่อง UI
  จึงแยกเป็นตั๋วที่ถาม #143
...
| `rubrics` | `21a-rubrics` | 16 ผ่าน | 2 ล้ม 14 ผ่าน | 1 ล้ม 15 ผ่าน (4 ตัว) |
| `criteria` | `22a-rubric-criteria` | 15 ผ่าน | 2 ล้ม 13 ผ่าน | 1 ล้ม 14 ผ่าน (4 ตัว) |
...
**ชื่อ spec ในคอลัมน์ที่สองเขียนเต็มเมื่อ 24 ก.ย. 2569 (#158)** สองช่องเคยเขียนสั้นว่า `21a` `22a`
ซึ่ง Playwright จับ `121a` (1 แถว) กับ `122a` (2 แถว) ด้วย **baseline ไม่ได้รวมสองไฟล์นั้น**:
16 = 11 แถวของ `21a` บวก 5 แถวของกลุ่ม rubrics เอง และ 15 = 10 บวก 5 ถ้าไฟล์ที่ชนได้รันด้วย
ทั้งคู่จะเป็น 17 ตรวจเมื่อ 24 ก.ย. 2569 ด้วยการบวกจำนวนแถว ไม่ได้กวาดใหม่
