...
| Rubrics | `21a-rubrics` | 23 ผ่าน | ล้ม 1 ผ่าน 22 (แถวแรก) | ล้ม 1 ผ่าน 22 (แถวที่สอง) | ล้ม 1 ผ่าน 22 (แถวที่สาม) |
| RubricCriteria | `22a-rubric-criteria` | 19 ผ่าน | ล้ม 1 ผ่าน 18 (แถวแรก) | ล้ม 1 ผ่าน 18 (แถวที่สอง) | ล้ม 1 ผ่าน 18 (แถวที่สาม) |
...
**ชื่อ spec ในคอลัมน์ที่สองเขียนเต็มเมื่อ 24 ก.ย. 2569 (#158)** `21a` กับ `22a` เคยเขียนสั้น ซึ่ง
Playwright จับ `121a` (1 แถว) กับ `122a` (2 แถว) ด้วย **baseline ไม่ได้รวมสองไฟล์นั้น** ทุกกลุ่ม
มากกว่าผลบวกของ spec ที่ระบุอยู่ **หนึ่งแถวเท่ากันหมด** รวมทั้งหกกลุ่มที่ไม่มีอะไรจะชนได้เลย -
ส่วนเกินหนึ่งนั้นจึงเป็นของทุกกลุ่ม (แถวตัวกรองของ `139a` ไม่มีชื่อไฟล์ในหัวข้อ `--grep-invert`
จึงตัดออกไม่ได้) Rubrics ได้ 23 = 22 + 1 ไม่ใช่ + 2 และ RubricCriteria ได้ 19 = 18 + 1 ไม่ใช่ + 3
ตรวจเมื่อ 24 ก.ย. 2569 ด้วยการบวกจำนวนแถว ไม่ได้กวาดใหม่
